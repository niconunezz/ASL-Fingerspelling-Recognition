...
class FeatureExtraction(nn.Module):
    def __init__(self, cfg, out_dim ,channels: int = 3, total_lm: int = 130, width: int = 130):
        super(FeatureExtraction, self).__init__()
        # self.in_channels  = 32 * math.ceil(total_lm / 2)
        ...
        output_size = math.ceil(total_lm / 2) * 32
        ...
        self.stem_linear = nn.Linear(output_size, out_dim, bias=False)
    ...
    def forward(self, data, mask, verbose = False):
        import time
        t0 = time.time()
        xc = data.permute(0, 3, 1, 2) # B, C, T, L
        t1 = time.time()
        if verbose:
            print(f"Permuting data took {(t1-t0)*1000:.2f} ms")

        t0 = time.time()
        xc = self.conv_stem(xc) # B, 32, T, L//2
        t1 = time.time()
        if verbose:
            print(f"Conv stem took {(t1-t0)*1000:.2f} Mseconds")

        t0 = time.time()
        xc = self.bn_conv(xc) # B, 32, T, L//2
        ...
        t0 = time.time()
        xc = xc.reshape(*data.shape[:2], -1) # B, T, 32 * W//2
        t1 = time.time()
        if verbose:
...
class Net(nn.Module):
    def __init__(self, config, decoder_cfg):
        ...
        total_lm = config.total_lm
        out_dim = config.out_dim
        n_land_el = config.n_land_el
        self.feature_extraction = FeatureExtraction(config, out_dim, total_lm = total_lm)
        # self.face_fe = FeatureExtraction(out_dim = 52, height=75)
        # self.pose_fe = FeatureExtraction(out_dim = 52, height=15)
        self.lhand = FeatureExtraction(config, out_dim//n_land_el, total_lm=total_lm//n_land_el)
        self.rhand = FeatureExtraction(config, out_dim//n_land_el, total_lm=total_lm//n_land_el)
    ...
    def forward(self, data, mask, targets, verbose = False):
        ...
        ccat = torch.cat([left_hand, right_hand], dim=2)
